...
fn main() {
    ...
    let diagnostics_window = video_subsystem
        .window("Diagnostics", DIAGNOSTICS_WINDOW_WIDTH, DIAGNOSTICS_WINDOW_HEIGHT)
        .position_centered()
        .build()
        .unwrap();
    ...
    let device: AudioDevice<MonoSound> = audio_subsystem
        .open_playback(None, &desired_spec, |spec| MonoSound { samples: vec![0; spec.samples as usize] })
        .unwrap();
    ...
    'running: loop {
        for event in event_pump.poll_iter() {
            // TODO match even close window exits program
